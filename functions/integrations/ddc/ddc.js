(async (APILoader) => {
  ...
  API.loadJS(
    "https://cdn.jsdelivr.net/gh/alireza-valizadeh/glo3d-script@110775def2a1798673f4dc0f91dbba5e8b6fda06/functions/integrations/base.js"
  ).then(() => async (APILoader) => {
    const API = await APILoader.create("glo3d");
    API.subscribe("vehicle-shown-v1", (ev) => {
      API.log("ev", ev);
      API.log("eventpayload.vin", ev.payload.vin);
