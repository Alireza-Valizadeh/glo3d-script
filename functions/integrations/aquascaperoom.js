...
jQuery(document).ready(function ($) {
  var vin_number = findVin();
  if (vin_number != "not_found") {
    var modelData = getModelDataFromGlo3D(vin_number);
  }
  ...
  function getModelDataFromGlo3D(vin_number) {
    var data = { type: "sku", vin_number: vin_number, height: "400" };
    $.ajax({
      type: "POST",
      ...
      replaceDefaultImage(result.shortId);
      add360Button();
    });
  }
  ...
  function addGloModal(shortId, width, height) {
    $("body").append(
      '<div id="myModalGlo3D" class="myModalGlo3D">\n' +
        "                <!-- Modal content -->\n" +
        '            <div class="modal-content-glo3d">\n' +
        '                <span class="close-glo">×</span>\n' +
        '                <iframe style="display: block; margin: 0 auto; width: ' +
        width +
        "; height: " +
        height +
